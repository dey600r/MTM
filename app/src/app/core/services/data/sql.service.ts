...
export class SqlService {
  ...
  getSqlVehicle(): string {
      ...
      `v.${ConstantsColumns.COLUMN_MTM_VEHICLE_ACTIVE}, ` +
        ...
        if (!!operation) { // if data exists it just save maintenance element
          ...
          operationsDB = [...operationsDB, {
            ...
            vehicle: new VehicleModel({
              model: row.modelVehicle,
              brand: row.brandVehicle,
              km: row.kmVehicle,
              configuration: null,
              vehicleType: this.getMapVehicleType(row, true),
              dateKms: null,
              datePurchase: null,
              active: (row.activeVehicle === Constants.DATABASE_YES),
              id: row[ConstantsColumns.COLUMN_MTM_OPERATION_VEHICLE]
            }),
  ...
  insertSqlVehicle(vehicles: VehicleModel[]): string {
    ...
    if (!!vehicles && vehicles.length > 0) {
      sql = `INSERT INTO ${ConstantsTable.TABLE_MTM_VEHICLE} ` +
        ...
        `${ConstantsColumns.COLUMN_MTM_VEHICLE_VEHICLE_TYPE}, ${ConstantsColumns.COLUMN_MTM_VEHICLE_ACTIVE}) `;
      vehicles.forEach((x, index) => {
        sql += `SELECT '${x.model}', '${x.brand}', ${x.year}, ${x.km}, ${x.configuration.id}, ` +
          ...
          `'${(x.active ? Constants.DATABASE_YES : Constants.DATABASE_NO)}' `;
  ...
  updateSqlVehicle(vehicles: VehicleModel[]): string {
    ...
    if (!!vehicles && vehicles.length > 0) {
      vehicles.forEach(x => {
        ...
        `${ConstantsColumns.COLUMN_MTM_VEHICLE_ACTIVE}='${(x.active ? Constants.DATABASE_YES : Constants.DATABASE_NO)}' ` +
