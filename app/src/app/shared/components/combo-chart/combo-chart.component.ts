...
export class ComboChartComponent extends BaseChartComponent {
  ...
  yLeftAxisScaleFactor(min: number, max: number): any {
    return { min: `${min}`, max: `${max}` };
  }

  yRightAxisScaleFactor(min: number, max: number): any {
    return { min: `${min}`, max: `${max}` };
  }

  yAxisTickFormatting(data: any): string {
    return `${data.toLocaleString()}`;
  }

  yRightAxisTickFormatting(data: any): string {
    return `${data}`; // %
  }
